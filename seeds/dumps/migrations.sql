...
INSERT INTO `minetop-reloaded`.`auths`(`email`, `password`, `created_at`, `updated_at`) 
...
INSERT INTO `minetop-reloaded`.`users`(`auth_id`, `pseudo`, `last_ip`, `last_login`) 
SELECT NEW_T.`id`, OLD_T.`name`, OLD_T.`lastip`, NEW_T.`updated_at`
FROM `minetop`.`account` AS OLD_T
JOIN `minetop-reloaded`.`auths` AS NEW_T ON OLD_T.`email` = NEW_T.`email`
WHERE OLD_T.`name` NOT IN ('Jill', 'Metra');

-- Ajout des credientials des réseaux sociaus liés aux comptes
INSERT INTO `minetop-reloaded`.`oauth_identities`(`user_id`, `provider_user_id`, `provider`, `created_at`, `updated_at`) 
...
(
    ...
    JOIN `minetop-reloaded`.`auths` AS A ON OLD_A.`email` = A.`email`
);

-- Rempli la nouvelle table des serveurs
INSERT INTO `minetop-reloaded`.`servers`(`top_server_id`, `name`, `slug`, `website`, `short_description`, `description`, `ip`, `port`, `slots`, `access`, `owner_id`, `is_enable`, `token`, `teamspeak_ip`, `teamspeak_port`, `mumble_ip`, `mumble_port`, `total_clics`, `total_votes`, `created_at`, `updated_at`) 
...
INSERT INTO `minetop-reloaded`.`monthly_stats`(`server_id`, `year`, `january_votes`, `february_votes`, `march_votes`, `april_votes`, `may_votes`, `june_votes`) 
...
INSERT INTO `minetop-reloaded`.`monthly_stats`(`server_id`, `year`, `july_votes`, `august_votes`, `september_votes`, `october_votes`, `november_votes`, `december_votes`) 
...
INSERT INTO `minetop-reloaded`.`server_banners`(`server_id`, `url`, `enable`) 
