...
CREATE TEMPORARY TABLE IF NOT EXISTS `minetop`.`full_servers` (id INTEGER NOT NULL AUTO_INCREMENT PRIMARY KEY) AS 
(
	SELECT
    ...
    FROM `minetop`.`topsite` AS T
	LEFT JOIN `minetop`.`stats` AS S ON T.`keygen` = S.`serial`
    JOIN `minetop`.`account` AS OLD_A ON OLD_A.`id` = T.`owner`
    JOIN `homestead`.`auths` AS A ON OLD_A.`email` = A.`email`
);

-- Rempli la nouvelle table des serveurs
INSERT INTO `homestead`.`servers`(`top_server_id`, `name`, `slug`, `website`, `short_description`, `description`, `ip`, `port`, `slots`, `access`, `owner_id`, `is_enable`, `token`, `teamspeak_ip`, `teamspeak_port`, `mumble_ip`, `mumble_port`, `total_clics`, `total_votes`, `created_at`, `updated_at`) 
...
FROM `minetop`.`full_servers`;

-- Insert les stats mensuels de l'année en cours
INSERT INTO `homestead`.`monthly_stats`(`server_id`, `year`, `january_votes`, `february_votes`, `march_votes`, `april_votes`, `may_votes`, `june_votes`) 
...
JOIN `homestead`.`servers` AS S ON S.`owner_id` = F.`owner_id`;

-- Insert les stats mensuels de l'année derniere
INSERT INTO `homestead`.`monthly_stats`(`server_id`, `year`, `july_votes`, `august_votes`, `september_votes`, `october_votes`, `november_votes`, `december_votes`) 
...
INSERT INTO `homestead`.`weekly_stats`(`server_id`, `year`, `number`, `monday_votes`, `tuesday_votes`, `wednesday_votes`, `thursday_votes`, `friday_votes`, `saturday_votes`, `sunday_votes`) 
...
INSERT INTO `homestead`.`server_banners`(`server_id`, `url`, `enable`) 
